...
/*
    ...
    什么时候成员变量声明为静态变量呢？
        - 所有对象都有这个属性，并且所有对象的这个属性的值是一样的，建议定义为静态变量，节省的内存的开销。
    ...
    关于java中的static关键字：
        ...
        2、static修饰的方法是静态方法
        3、static修饰的变量是静态变量
        4、所有static修饰的元素都成为静态的，都可以使用"类名."的方式访问，当然也可以用引用.的方式访问【但不建议】
...
public class Chinese {
    ...
    //实例变量【实例变量一个java对象就有一份，100个java对象，就有100个country0】，分析这种设计方式有什么缺点？
    //实例变量存储java对象内部，在堆内存当中，在构造方法执行的时候初始化。
    //所有的中国人的国籍都是"中国"，这里声明为实例变量显然是不合适的，太浪费内存空间，没必要让每一个对象都保留一份"国籍"内存。
    //所有对象国籍一样，这种特征属于类级别的特征，可以提升为整个模板的特征，可以在变量前添加static关键字修饰
