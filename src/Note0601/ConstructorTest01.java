...
/*
    关于java类中的构造方法：
        ...
        7、构造方法应该怎么调用？
            - 普通方法：方法修饰符中有static的时候：类名.方法名（实参列表） 或者方法修饰符中没有static的时候： 引用.方法名（实参列表）
            - new 构造方法名（实参列表）
        8、构造方法调用执行之后，有返回值吗？
            每一个构造方法实际上执行结束后都有返回值，但是这个“return 值”；这样的语句不需要写。构造方法结束的时候java程序自动返回值。
            并且返回值类型是构造方法所在类的类型。由于构造方法的返回值类型就是类本身，所以返回值类型不需要编写。
        9、注销和取消注释：ctrl+/，多行注释：ctrl+shift+/
        10、当一个类中没有定义任何构造方法的话，系统默认给该类提供一个无参数的构造方法，这个构造方法称为缺省构造器。
...
public class ConstructorTest01 {
    public static void main(String[] args) {
        ...
        //调用没有static的方法：引用.
        //doOther方法在ConstructorTest01当中，所以需要创建ConstructorTest01对象
