...
public class Test {
    public static void main(String[] args) {
        ...
        /*
            ...
            3、new Cat()创建的对象的类型是cat，a2这个引用的数据类型是Animal，课件他们进行了类型转换
            子类型转换成父类型，称为向上转型/upcasting，或者称为自动类型转换。
            4、Java中允许这种语法：父类型引用指向子类型对象。
         */
        Animal a2=new Cat();
        //Bird b2=new Cat();编译报错，因为两种类型不存在任何继承关系。无法向上或者向下转型。
        /*
            1、程序永远都分为编译阶段和运行阶段。
            2、先分析编译阶段，再分析运行阶段，编译无法通过，根本是无法运行的。
            3、编译阶段编译器检查a2这个引用的数据类型为Animal，由于Animal.class字节码当中有move()方法，
            所以编译通过了。这个过程我们成为静态绑定，编译阶段绑定。
            只有静态绑定成功之后才有后续的运行。
            4、在程序运行阶段，JVM堆内存当中真是创建的对象是Cat对象，那么一下程序在运行阶段
        ...
        /*
            需求：
                假设想让以上的对象执行catchMouse()方法，怎么办？
                    ...
                    我们可以将a2强制类型转换为Cat类型。
                    a2的类型是Animal(父类)，转换成Cat类型(子类)，被称为向下转型downcasting/强制类型转换。
        ...
        Animal a3=new Bird();
        /*
            1、以下程序编译时没有问题的，因为编译器检测到a3的数据类型是Animal
            向下转型，语法合格。
        ...
        /*
            ...
            3、向下转型编译通过，运行可能错误：Animal a2=new Bird(); Cat c3=(Cat) a3;
            4、怎么避免向下转型出现的ClassastException呢？
                使用instanceof运算符可以避免出现以上的异常。
            5、instanceof运算符怎么用？
                5.1、语法格式引用 (引用 instanceof 数据类型名)
            ...
            6、Java规范要求：在进行强制类型转换之前，建议采用instanceof运算符进行判断，避免ClassCastException
         */
